...
import autonomouspathplanner.GUI.*;
...
public class FalconPathPlanner
{
	...
	double velocityTolerance;
        ...
        static final double thetaTolerance = 0.01;
	 ...
	 * double[][] waypointPath = new double[][]{
		...
		This path goes from {1,1} to {5,1} to {9,12} to {12, 9} to {15,6} to {15,4}
	 ...
	 * @return a copied version of <code>arr</code>
	 */
	 ...
	 * @param numToInject number of samples to inject
	 * @return a new path with injected points
	 */
	 ...
	 * 
	 * @param path the path to smoothen out
	 * @param weight_data how much weight to give the initial path
	 * @param weight_smooth how much weight to give the smoothness
	 * @param tolerance how close is close enough?
	 * @return a path that is smoother than <code>path</code>
	 */
	 ...
	 * @return path with only direction changing nodes
	 */
	public static double[][] nodeOnlyWayPoints(double[][] path)
	{
		...
		for(int i=1; i<path.length-1; i++)
		{
			...
			if(Math.abs(vector2-vector1)>=thetaTolerance) 
				li.add(path[i]);					
		}
	 ...
	 * Big O: Constant Time
	 * 	
	 * @param numNodeOnlyPoints number of nodes in the node only arrray
	 * @param maxTimeToComplete the total time allowed for the path to run
	 * @param timeStep the loop time of the robot.
         * @return the amount of nodes to inject (and where? i guess?)
	 */
	public int[] injectionCounter2Steps(double numNodeOnlyPoints, double maxTimeToComplete, double timeStep)
	{
		...
		double totalPoints = maxTimeToComplete/timeStep;
                ...
                System.out.println(totalPoints + " " + maxTimeToComplete);
			...
			for (int i=4; i<=6; i++)
				...
				for (int j=1; j<=8; j++)
				{
					...
					if(pointsTotal<=totalPoints && pointsTotal>oldPointsTotal)
					{
                        ...
                        /*
                        if((first + second)<totalPoints)
                        {
                            
                            third = (int) (totalPoints - (first+second));
                        }*/
			...
			for (int i=1; i<=5; i++)
				for (int j=1; j<=8; j++)
					for (int k=1; k<8; k++)
					{
                        ...
                        if(totalPoints>pointsTotal)
                        {
                            third += (int) (totalPoints - pointsTotal);
                        }   
			ret = new int[] {first, second, third};
		}
                int total = first + second + third;
                
                System.out.println("NumPoints to inject is " + total);
 ...
 */
	...
	public void calculate(double totalTime, double timeStep, double robotTrackWidth)
	{
		...
		leftRight(smoothPath, robotTrackWidth);
                ...
                //Use chezy pof files in order to calculate velocity - this will get us places?
